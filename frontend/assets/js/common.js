...
//    window.location.href="http://localhost/tweety/home";
// });

$(document).on("click",".notify-msg-container",function(){
   var otherid=$(this).data('profileid');
   if(otherid !=undefined){
    window.location.href="http://localhost/tweety/messages/"+otherid;
   }
})
...
$("#submitPostButton").click(e=>{
  ...
  if((textValue !="" && textValue != null) && postImage ==null){
    $.post("http://localhost/tweety/backend/ajax/post.php",{onlyStatusText:textValue,userid:userid},function(data){
     ...
     $.ajax({
       url:"http://localhost/tweety/backend/ajax/post.php",
       type:"POST",
       cache:false,
       processData:false,
       data:formData,
