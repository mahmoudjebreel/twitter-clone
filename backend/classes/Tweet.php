...
class Tweet{
    ...
    public function tweets($user_id,$num){
       ...
       foreach($tweets as $tweet){
            ...
            $controls=$tweetControls->createControls($tweet->tweet_id,$tweet->tweetBy,$user_id);
              ...
              <div class="retweet-user-link">
                ...
                <div class="postContentContainer">
                    <div class="post-header">
                        ...
                        '.(($tweet->tweetBy===$user_id) ? '<span class="dot deletePostButton" id="deleteButton" data-tweetid="'.$tweet->tweet_id.'" data-tweetby="'.$tweet->tweetBy.'" data-userid="'.$user_id.'">
    ...
    public function getHashtagTweets($hashtag,$user_id){
        $stmt=$this->pdo->prepare("SELECT * FROM users u LEFT JOIN tweets p ON p.tweetBy=u.user_id INNER JOIN trends t ON p.tweet_id=t.tweetId WHERE hashtag=:hashtag ORDER BY postedOn DESC");
   ...
   public function tweetCounts($profileId){
    ...
    public function getPopupTweet($tweetId,$tweetBy){
        $stmt=$this->pdo->prepare("SELECT * FROM `tweets` LEFT JOIN `users` ON users.user_id=tweets.tweetBy WHERE tweets.tweet_id=:tweetid AND tweets.tweetBy=:tweetby");
        $stmt->bindParam(":tweetid",$tweetId,PDO::PARAM_INT);
    ...
    public function trends(){
        $stmt=$this->pdo->prepare("SELECT *,COUNT(`tweet_id`) AS `tweetsCount` FROM trends t LEFT JOIN tweets p ON p.tweet_id=t.tweetId AND status LIKE CONCAT('%#',`hashtag`,'%') GROUP BY `hashtag` ORDER BY `tweetsCount` DESC LIMIT 3");
        $stmt->execute();
